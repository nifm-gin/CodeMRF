function [ T1list, T1, T2list, T2, B1rellist, B1rel, dflist, df ] = genproperties( niveau )
%GENPROPERTIES Summary of this function goes here
...
elseif niveau == 4
    ...
    [T2mesh,T1mesh,dfmesh,B1relmesh]=ndgrid(T2,T1,df,B1rel);
    indices=find(T1mesh(:)>T2mesh(:));
    T1list=T1mesh(indices(:));
    T2list=T2mesh(indices(:));
    dflist=dfmesh(indices(:));
    B1rellist=B1relmesh(indices(:));
